...
const { work_folder } = config.settings;
...
const WorkSingle = ({ params }: { params: { single: string } }) => {
  const works: Work[] = getSinglePage(work_folder);
  const work = works.filter((page) => page.slug === params.single)[0];
  ...
  const similarWorks = similerItems(work, works, work.slug!);
      ...
      <section className="section pt-7">
        <div className="container">
          ...
          {/* <div className="section pb-0">
            <h2 className="h3 mb-12 text-center">Related Works</h2>
            <div className="row justify-center">
              {similarWorks.map((work) => (
                <div key={work.slug} className="lg:col-4 mb-7">
                  <WorkCard data={work} />
                </div>
              ))}
            </div>
